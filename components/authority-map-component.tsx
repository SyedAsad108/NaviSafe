    ...
    if (typeof window !== "undefined" && mapRef.current && !map) {
      ...
      import("leaflet").then((L) => {
        ...
        L.Icon.Default.mergeOptions({
          iconRetinaUrl: "/map-marker-icon.jpg",
        ...
        restrictedZones.forEach((zone) => {
          ...
          L.circle([zone.lat, zone.lng], {
            ...
            weight: 4,
            radius: zone.radius,
            className: zone.risk === "CRITICAL" ? "animate-pulse" : "",
          })
            .addTo(mapInstance)
            .bindPopup(`
              <div style="min-width: 200px;">
                <strong style="color: ${color};">⚠️ ${zone.name}</strong><br/>
        ...
        touristLocations.forEach((tourist) => {
          ...
          const statusIcon = tourist.safeScore >= 80 ? "👤" : tourist.safeScore >= 60 ? "⚠️" : "🚨"

          L.marker([tourist.lat, tourist.lng], {
            icon: L.divIcon({
              className: "custom-div-icon",
              html: `<div style="background-color: ${iconColor}; color: white; border-radius: 50%; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: bold; border: 3px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3); ${tourist.safeScore < 80 ? "animation: pulse 2s infinite;" : ""}">${statusIcon}</div>`,
              iconSize: [28, 28],
              iconAnchor: [14, 14],
            }),
          })
        ...
        incidents.forEach((incident) => {
          ...
          L.marker([incident.location.lat, incident.location.lng], {
            icon: L.divIcon({
              className: "custom-div-icon",
              html: `<div style="background-color: ${color}; color: white; border-radius: 50%; width: ${size}px; height: ${size}px; display: flex; align-items: center; justify-content: center; font-size: ${size / 2}px; font-weight: bold; border: 4px solid white; box-shadow: 0 3px 10px rgba(0,0,0,0.4); animation: ${incident.priority === "HIGH" || incident.priority === "CRITICAL" ? "pulse 1.5s infinite" : "none"};">${icon}</div>`,
              iconSize: [size, size],
        ...
        policeStations.forEach((station) => {
          L.marker([station.lat, station.lng], {
            icon: L.divIcon({
              className: "custom-div-icon",
              html: `<div style="background-color: #059669; color: white; border-radius: 6px; padding: 6px 10px; font-size: 12px; font-weight: bold; border: 3px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3); min-width: 50px; text-align: center;">🚔<br/>${station.units}</div>`,
              iconSize: [50, 30],
              iconAnchor: [25, 15],
            }),
          })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        <div
          ref={mapRef}
          className="h-[500px] w-full rounded-lg border border-border shadow-lg"
